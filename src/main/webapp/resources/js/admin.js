$(document).ready(function() {
	...
	$("a#groupadd").bind({
		"click" : function(e) {
			...
			$("div#grpList").addClass('hide');
			$("div#empUpdate").addClass('hide');
			$("div#grpSearch input[type='submit']").value = "Save";
...
function bindUpdateRows() {
	$("table").on("click",".updateRow",
			function(e) {
							var table = $.fn.dataTable.fnTables(true),
							bindingData = getFormData(e,table,"update"), divToShow = bindingData.div, url = bindingData.url;
							$
									.ajax({
										url : url,
										type : "get",
...
function bindDeleteRows() {
	$("table").on("click",".deleteRow",	
					 function(e) {
							var table = $.fn.dataTable.fnTables(true),
							bindingData = getFormData(e,table,"delete"), divToShow = bindingData.div, url = bindingData.url;
							$
									.ajax({
										url : url,
										type : "POST",
...
function getFormData(e,table, action) {
	...
	data = $(table).dataTable()._(row), id = data[0].id , retArgs = {};
	if (tableId.indexOf("grpListTable") >= 0) {
		retArgs['url'] = "/matrix/admin/group/" + action + "?id=" + id;
		retArgs.div = 'grp' + capitaliseFirstLetter(action);
	} else {
		retArgs['url'] = "/matrix/admin/employee/" + action + "?id=" + id;
